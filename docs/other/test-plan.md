...
| T1.2 | 全特性构建 | `cargo build --features "default,plugin-system"` | ✅ 通过 |
| T1.3 | 插件系统构建 | `cargo build --features "default,plugin-system"` | ✅ 通过 |
| T1.4 | 最小构建 | `cargo build --no-default-features --features adapter-telegram` | ✅ 通过 |
| T1.5 | 无适配器构建 | `cargo build --no-default-features` | ✅ 通过 |
| T1.6 | release 构建 | `cargo build --release` | ✅ 通过 |
| T1.7 | Clippy | `cargo clippy --all-targets --features "default,plugin-system"` | ✅ 无严重 lint |
| T1.8 | 单元测试 | `cargo test --workspace --lib --features "default,plugin-system"` | ✅ 全部通过 |
...
| T3.9.2 | 更新配置 | `PUT /api/v1/config` | ✅ 409 拒绝部分热更新并审计；配置通过滚动重启生效 |
...
cargo clippy --all-targets --features "default,plugin-system" || true

echo "=== T1.8: 单元测试 ==="
cargo test --workspace --lib --features "default,plugin-system"
...
echo "=== T6: 插件加载测试 ==="
cargo test --workspace --lib --features "default,plugin-system" plugin:: ffi::
