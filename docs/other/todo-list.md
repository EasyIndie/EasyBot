...
| **Windows U2 升级真机验证** | `docs/other/windows-deployment.md` §9 | 分离辅助脚本两步替换的运行时锁语义（marker OK/TIMEOUT、`!`/空格路径、回滚拒绝）已通过 Windows 真机 + NSSM 验收（2026-08-14：U1、U2 分离交换机制、场景 A/B/C/D/E/F/G 全部端到端——A/B/D 在 v0.0.37 发版后补全；G 因 v0.0.37 无 DB 迁移不适用，由 v0.0.36 的 v2→v3 迁移覆盖）；验收结论见 windows-deployment.md「真机验证结论」 |
