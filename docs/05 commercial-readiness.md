...
动态插件通过进程内 `dlopen` 执行，ed25519 签名与发布者信任已实现（`core/src/plugin/signing/`），但插件**无沙箱**——签名只证作者与完整性，不证代码安全，插件以宿主权限运行。生产门禁在启动时扫描 `plugins/` 签名：存在未签名插件且未设 `allowUntrusted` 时拒绝启动。承载不可信插件须用容器化隔离兜底；生产隔离优先将扩展部署为独立隔离服务。
...
生产 Compose 还必须验证 Caddy 能通过容器内网访问 EasyBot；生产 override 会将应用监听地址设为 `0.0.0.0`，而不是让应用直接暴露宿主机公网端口。
