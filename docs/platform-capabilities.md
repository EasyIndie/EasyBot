...
**Discord Components 消息格式**（ActionRow + Button）:
```json
{
  "flags": 32768,
  "components": [
...
- **事件推送模式**: **集群模式**——飞书开放平台的 WebSocket 事件订阅仅随机推送到**其中一个**连接的客户端。同一飞书应用的多个 WebSocket 连接不会都收到同一事件。部署多个 EasyBot 实例时需注意此限制
...
> **API 注意点**: QQ C2C 私聊不支持 `msg_type: 2`（图文混合消息），发送图片到 C2C 需降级为 `msg_type: 1`（纯图片）或消息段模式。编辑/删除消息仅限频道消息（`PATCH /channels/{id}/messages/{msg_id}` / `DELETE /channels/{id}/messages/{msg_id}`），C2C/群聊不支持。
...
- **Outbound 能力**: Text / Image / Audio / Video / Document 全部已实现（AES-128-ECB 加密 + CDN 上传）。编辑/删除/交互式按钮/聊天列表 — iLink Bot API 不支持
...
> ⚠️¹ QQ Thread: 适配器声明支持，但入站消息 `thread_id` 未填充，无实际线程路由实现。QQ 频道子频道概念存在，但适配器未实现线程感知的消息路由。
> ⚠️² QQ ChatList: 仅支持列出频道服务器 (Guild)，QQ 群聊和 C2C 私聊无法通过 API 枚举。
...
| **QQ** | Gateway WebSocket | `AT_MESSAGE_CREATE` / `GROUP_AT_MESSAGE_CREATE` / `GROUP_MESSAGE_CREATE` / `C2C_MESSAGE_CREATE` Dispatch |
