...
| T1.8 | 单元测试 | `cargo test --workspace --lib --features "full,plugin-system"` | **284 tests passed** |
| T1.9 | 集成测试 | `cargo build -p mock-adapter && cargo test -p integration-tests` | **8 tests passed** (Plugin system E2E) |
...
| T3.5.5 | 发送媒体消息 | `POST /api/v1/messages/send` `{"target":"wechat:user","text":"caption","media":{"url":"https://example.com/img.png","type":"image"}}` | `200` 含消息 ID，适配器执行 CDN 上传 |
| T3.5.6 | 非法 media URL | 同上，`media.url:"invalid"` | 适配器返回错误，响应含 error 字段 |
...
| T4.2.6 | 事件类型 | `event_types::all()` | 包含所有 12 个预定义类型 |
...
| T4.6.7 | send_media 到未连接适配器 | 返回 `AdapterNotConnected` |
| T4.6.8 | start_all 跳过 disabled | enabled:false 的适配器不启动 |
| T4.6.9 | start_all 部分失败 | 返回 succeeded + failed |
| T4.6.10 | stop_all | 所有适配器 disconnect |
...
| T8.8 | adapter.enabled=false | 适配器配置 | 不启动对应适配器 |
...
| SQLite Storage | ✅ 13 tests | — | — |
| PostgreSQL | ✅ (feature-gated) | — | T5.2 |
| TTL Retention | ✅ 3 tests | — | — |
| 速率限制 | ✅ 6 tests | — | T3.3 |
| 配置加载 | ✅ 26+ tests | — | T8 |
| SessionManager | ✅ 5 tests | — | T3.7 |
| 插件 Manifest | ✅ 7 tests | — | — |
| 插件加载 | ✅ 11 tests | ✅ 9 tests | T6.1 |
...
**总计：284 单元测试 + 8 集成测试 + ~60 手动/e2e 验收点**
